...
def playGame(p1, p2, f):
    ...
    while p1len != 0 or p2len != 0: # established the endgame rules for War, where one player needs the whole deck to win
        ...
        if winner == "p1": #decides the winner and gives the pot to who won
            ...
            f.write("Player 1 has won round " + str(count) +"!  Player 1 now has " + str(len(p1)) + " cards.  Player 2 now has " + str(len(p2)) + " cards.<br>")
        else:
            for i in range(0, len(currentPot)):
                p2.append(currentPot[i])
            f.write("Player 2 has won round " + str(count) +"!  Player 1 now has " + str(len(p1)) + " cards.  Player 2 now has " + str(len(p2)) + " cards.<br>")
